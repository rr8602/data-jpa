...
class MemberRepositoryTest {
    ...
    public void testMember(){
        // JPARepository의 구현 클래스를 Proxy로 Spring Data Jpa에서 만들어주고, injection 해준다
        // memberRepository = class jdk.proxy2.$Proxy122 출력
...
//        // 변경 감지로 인한 update query 자동으로 나감
//        findMember1.setUsername("member!!!!!!!!!!!");
    ...
    @Test
    public void findByUsernameAndAgeGreaterTahn(){
    ...
    public void findMemberDto() {
        ...
        member.setTeam(team); // 연관관계 상, member에도 team의 내용이 반영되어 있어야 한다. (데이터 정합성)
        ...
        assertThat(memberDto.get(0).getId()).isEqualTo(1);
    ...
    @Test
    public void bulkUpdate(){ // 벌크 연산은 영속성 컨텍스트를 무시하고, 곧바로 DB에 반영을 시킨다. 그래서 반드시 벌크 연산 이후에는 영속성 컨텍스트를 flush하고 clear 해줘야 한다.
        //given
        memberRepository.save(new Member("member1", 10));
...
//        em.flush(); // DB에 반영, commit 전이나 JPQL 실행 전에 자동 flush가 됨
//        em.clear(); // 영속성 컨텍스트 clear
    ...
    public void queryByExample(){
        ...
        Team team = new Team("teamA"); // 내주보인으로 teamA 가능
        member.setTeam(team);

        ExampleMatcher matcher = ExampleMatcher.matching()
                .withIgnorePaths("age"); // primitive 타입인 int가 null이 아니라서 ignore하는 과정
...
} // PageRequest는 Pageable의 구현체, Page는 count 쿼리 결과를 포함하는 페이징 반환 타입
